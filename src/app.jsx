...
    const [deployments, setDeployments] = useState(initialDeployments); // Dynamic project list
...
        if (!userQuery) {
            setNarrativeOutput(<p className="text-vvv-coral">Error: Please enter a business concept or challenge to begin orchestration.</p>);
            return;
        }
...
            } else {
                setNarrativeOutput(<p className="text-vvv-coral">Error: Failed to generate narrative. Response structure was invalid.</p>);
            }

        } catch (error) {
            console.error("Gemini API Call Error:", error);
            setNarrativeOutput(<p className="text-vvv-coral">System Error: Connection failed or maximum retries exceeded. Check console for details.</p>);
        } finally {
            setIsLoading(false);
        }
    };

    // --- Tailwind/Global Styles (Embedded in React) ---
    const globalStyles = `
        /* Global CSS Variables from Manifesto */
        :root {
            --vvv-purple: ${VVV_COLORS.purple};
            --vvv-coral: ${VVV_COLORS.coral};
            --vvv-charcoal: ${VVV_COLORS.charcoal};
            --vvv-surface: ${VVV_COLORS.surface};
            --vvv-text: ${VVV_COLORS.text};
            --vvv-muted: ${VVV_COLORS.muted};
            --vvv-divider: ${VVV_COLORS.divider};
            --vvv-grad: linear-gradient(90deg, var(--vvv-coral) 0%, var(--vvv-purple) 100%);
        }

        /* Tailwind requires pre-registration for custom hex values when using utility classes dynamically. */
        /* Since we can't use a build tool like PostCSS, we manually define utility classes here. */
        .bg-vvv-surface { background-color: var(--vvv-surface); }
        .bg-vvv-charcoal { background-color: var(--vvv-charcoal); }
        .border-vvv-divider { border-color: var(--vvv-divider); }
        .hover\\:border-\\[\\#6246EA\\]:hover { border-color: var(--vvv-purple); }
        .hover\\:border-\\[\\#E9622D\\]:hover { border-color: var(--vvv-coral); }
        .text-vvv-text { color: var(--vvv-text); }
        .text-vvv-muted { color: var(--vvv-muted); }
        .text-vvv-purple { color: var(--vvv-purple); }
        .text-vvv-coral { color: var(--vvv-coral); }


        /* Apply background and font globally */
        body {
            background-color: var(--vvv-charcoal);
            color: var(--vvv-text);
...
        /* Coral Button Styles */
        .btn-coral {
            background-color: var(--vvv-coral);
            transition: background-color 0.3s ease, transform 0.1s ease;
            box-shadow: 0 4px 6px -1px rgba(233, 98, 45, 0.4), 0 2px 4px -2px rgba(233, 98, 45, 0.4);
        }
        .btn-coral:hover {
            background-color: #f77a4a;
...
        .btn-purple {
            background-color: var(--vvv-purple);
            transition: background-color 0.3s ease, transform 0.1s ease;
            box-shadow: 0 4px 6px -1px rgba(98, 70, 234, 0.4), 0 2px 4px -2px rgba(98, 70, 234, 0.4);
        }
...
        /* Loading Spinner */
        .loader {
            border: 4px solid var(--vvv-surface);
            border-top: 4px solid var(--vvv-coral);
            border-radius: 50%;
            width: 20px;
            height: 20px;
...
                        <p className="text-sm text-vvv-muted">Scalable systems built for speed. The fusion of logic and velocity.</p>
                    </div>
                    {/* Card 2: Artistry */}
                    <div className="bg-vvv-surface p-6 rounded-xl border border-vvv-divider">
...
                        <p className="text-sm text-vvv-muted">Visual and linguistic elegance. Every asset is a designed sequence.</p>
                    </div>
                    {/* Card 3: Precision */}
                    <div className="bg-vvv-surface p-6 rounded-xl border border-vvv-divider">
...
                        <p className="text-sm text-vvv-muted">Strategy disguised as design. Flawless execution where logic meets goal.</p>
                    </div>
                </section>
...
                            **Automation in Action:** Test our core value of Linguistic Art by generating a **Cinematic System Narrative** for a business challenge.
                        </p>

                        <div className="mb-6">
                            <label htmlFor="concept-input" className="block text-sm font-medium text-vvv-muted mb-2">
                                Input Business Challenge (e.g., "Need to automate content creation for a new product launch.")
                            </label>
                            <input 
                                type="text" 
                                id="concept-input" 
...
                        <div id="narrative-output" className="min-h-[6rem] bg-vvv-charcoal p-4 rounded-lg border border-vvv-divider flex items-center">
                            {narrativeOutput}
                        </div>
                    </div>
                </section>
...
                            Let's discuss how to merge **Automation** and **Artistry** for your next high-impact content system.
                        </p>
                        <a href="#" className="btn-coral text-vvv-text font-semibold py-3 px-10 rounded-lg shadow-lg inline-block text-lg">
                            Start the Dialogue
                        </a>
                    </div>
                </section>
...
                    <span className="text-vvv-purple">Automation</span> | <span className="text-vvv-purple">Artistry</span> | <span className="text-vvv-purple">Precision</span>
                </p>
            </footer>
        </>
    );
